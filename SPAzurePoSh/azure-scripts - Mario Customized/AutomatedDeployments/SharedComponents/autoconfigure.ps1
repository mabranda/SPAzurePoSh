...
#This is the main function. Verifies and creates accounts as needed.
Function AutoConfigure
{
    ...
    {
		#Need to check for existence of the storage account even if the param has a value
		if ((Get-AzureStorageAccount -StorageAccountName $storageAccountName) -ne $null)
		{
...
#Here's where we actually start creating stuff...
...
{
    param($configPath,$serviceName,$storageAccount,$subscription, $adminAccount, $password, $domain, $dnsDomain)
    $sql2k12img = (GetLatestImage "SQL Server 2012 SP1 Enterprise on Windows Server 2012")
    $configPathAutoGen = $configPath.Replace(".xml", "-AutoGen.xml")
    [xml] $config = gc $configPath
    $config.Azure.SubscriptionName = $subscription
    $config.Azure.ServiceName = $serviceName
